...
const AmqpMessage_1 = require("./AmqpMessage");
...
class AmqpServer {
                        ...
                        if (metaData) {
                            ...
                            this.handlersBucket[metaData.queue] = {
                                config: metaData, handler: (args) => {
                                    let instance;
                                    if (hasConstructor) {
                                        instance = this.buildController(consumer, new AmqpMessage_1.AmqpMessage(args));
                                    }
                                    else {
                                        instance = new consumer();
                                    }
                                    const finalArgs = this.buildArgs(instance, prop, new AmqpMessage_1.AmqpMessage(args));
    ...
    publishMessage(queue, message, json = true) {
